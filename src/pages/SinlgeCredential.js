...
export default function SingleCredentials({ history, match }) {
  ...
  const fair_id = new URLSearchParams(history.location.search).get('fair_id');
  const user_id = new URLSearchParams(history.location.search).get('user_id');
    ...
    <StyledDiv>
      ...
      <div className="cards-div">
        ...
        <div className="btns-div">
          <span aria-hidden id="approved" onClick={() => approval(true)}>Approved</span>
          <span aria-hidden id="rejected" onClick={() => approval(false)}>Rejected</span>
        </div>
      </div>
    </StyledDiv>
  );
